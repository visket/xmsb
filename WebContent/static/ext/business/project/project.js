var project_path;
var init_project_list_datagrid = function(path){
	project_path=path;
	$('#project_list_datagrid').datagrid({
		url: path+'/project/find',
		fit:true,
		border:false,
		pagination:true,
		pagePosition:'bottom',
	...
	top.$.messager.confirm('确认', '是否发起项目申报？', function(b) {
        if (b) {
            ...
            $.post(project_path + '/project/projectDeclare', {
                id:rows[0].id, unitId:rows[0].unitId, creatorId:userId, logId:rows[0].logId
            }, function(result) {
