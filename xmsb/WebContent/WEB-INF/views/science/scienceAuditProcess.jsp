...
<title>科技项目审核-查看所有流程状态</title>
	<script type="text/javascript"> 
		...
		$(function(){
			...
			thisrow = row;
	    	init_scienceAuditProcess_list_datagrid('${path}', thisrow.projectId);
	    	
	    });
	    
		function init_scienceAuditProcess_list_datagrid(path, projectId){
			science_path=path;
			$('#scienceAuditProcess_list_datagrid').datagrid({
				url: path+'/science/auditProcess',
				fit:true,
				border:false,
				pagination:true,
				pagePosition:'bottom',
				...
				pageNumber:1,
				fitColumns: true,
				toolbar:'#scienceAuditProcess_datagrid_toolbar',
				queryParams : {
					projectId : projectId
				}, 
				columns : [[
		...
		function auditmethods() {
			var rows = $('#scienceAuditProcess_list_datagrid').datagrid('getRows');
			var lastRow = rows[0];
			//校验身份
			...
			var str ="<div><iframe id='scienceAuditProcess_audit' src='${path}"
				+"/science/beforeAudit?projectId="+thisrow.projectId
				+"&&actualApproverId="+currentUserId
				+"&&logId="+thisrow.logId
				+"&&logStatusId="+thisrow.logStatusId
				+"&&operateType=audit"
				+"' width='100%' height='100%' style='border:0'></iframe></div>";
			loaddialogbyaudit('scienceAuditProcess_audit', str, thisrow.projectName + '-审核操作', 600, 270, type, 'scienceAuditProcess_list_datagrid', thisrow);
		}
		...
		function disponseOffice() {
			...
			var str ="<div><iframe id='scienceAuditProcess_list_disponseOffice' src='${path}"
				...
				+"&&operateType=office"
				+"' width='100%' height='100%' style='border:0'></iframe></div>";
				loaddialogbyscienceaudit('scienceAuditProcess_list_disponseOffice', str, thisrow.projectName + '-分发处室', 600, 350, type, 'scienceAudit_list_datagrid', thisrow);
		}
		...
		function disponseExpert() {
			...
			var str ="<div><iframe id='scienceAuditProcess_list_disponseOffice' src='${path}"
				...
				+"&&operateType=expert"
				+"' width='100%' height='100%' style='border:0'></iframe></div>";
				loaddialogbyscienceaudit('scienceAuditProcess_list_disponseOffice', str, thisrow.projectName + '-分发专家', 600, 350, type, 'scienceAudit_list_datagrid', thisrow);
		}
...
<body>
	<div class="easyui-layout" data-options="fit:true,border:false">
	    <div data-options="region:'center',border:false" style="width: 100%;">
	        <table id="scienceAuditProcess_list_datagrid" data-options="fit:true,border:false"></table>
	    </div>
	</div>
</body>
